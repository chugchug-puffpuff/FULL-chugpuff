import React from "react";
import "./style.css";

export const Element = () => {
  return (
    <div className="element">
      <div className="div">
        <div className="frame-2">
          <div className="text-wrapper">최신순</div>
          <img
            className="img"
            alt="Keyboard arrow up"
            src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66ba069ad632e20f0c1152a0/img/keyboard-arrow-up@2x.png"
          />
        </div>
<<<<<<< HEAD
        <div className="frame-3">
          <div className="frame-4">
            <div className="text-wrapper">최신순</div>
          </div>
          <div className="frame-4">
            <div className="text-wrapper">마감 임박순</div>
          </div>
        </div>
=======
>>>>>>> ca63ab59f84b3bee18722590476cbe8f39143013
      </div>
      <div className="text-wrapper-2">스크랩한 공고</div>
      <div className="frame-wrapper">
        <div className="frame-5">
          <div className="frame-6">
            <div className="text-wrapper-3">전체 (3건)</div>
          </div>
          <div className="frame-7">
            <div className="frame-8">
              <div className="frame-9">
                <img
                  className="img"
                  alt="Check box outline"
                  src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/check-box-outline-blank@2x.png"
                />
                <div className="text-wrapper-4">선택한 공고 삭제</div>
                <div className="frame-10">
                  <div className="text-wrapper">최신순</div>
                  <img
                    className="img"
                    alt="Keyboard arrow down"
                    src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/keyboard-arrow-down@2x.png"
                  />
                </div>
              </div>
              <img
                className="line"
                alt="Line"
                src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/line-2.png"
              />
            </div>
            <div className="frame-11">
              <div className="frame-12">
                <img
                  className="img"
                  alt="Check box outline"
                  src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/check-box-outline-blank-1@2x.png"
                />
                <div className="frame-13">
                  <div className="frame-14">
                    <div className="frame-15">
                      <div className="text-wrapper-5">(주)엔미디어플랫폼</div>
                      <img
                        className="img"
                        alt="Favorite"
                        src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/favorite@2x.png"
                      />
                    </div>
                    <p className="p">등록 10/3(월) ~ 마감 11/20(수)</p>
                  </div>
                  <div className="frame-16">
                    <div className="frame-17">
                      <div className="frame-18">
                        <p className="text-wrapper-6">Nexon Company window application 개발 엔지니어 모집</p>
                        <img
                          className="grade"
                          alt="Grade"
                          src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/grade@2x.png"
                        />
                      </div>
                      <div className="frame-19">
                        <div className="frame-20">
                          <div className="text-wrapper-7">3년 경력</div>
                          <img
                            className="img"
                            alt="Arrow upward alt"
                            src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/arrow-upward-alt@2x.png"
                          />
                        </div>
                        <div className="frame-20">
                          <div className="text-wrapper-7">학력 무관</div>
                        </div>
                        <div className="frame-20">
                          <div className="text-wrapper-7">서울 강남구</div>
                        </div>
                        <div className="text-wrapper-7">정규직</div>
                      </div>
                    </div>
                    <div className="frame-21">
                      <div className="text-wrapper-8">지원하기</div>
                    </div>
                  </div>
                </div>
              </div>
              <img
                className="line"
                alt="Line"
                src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/line-2.png"
              />
              <div className="frame-12">
                <img
                  className="img"
                  alt="Check box outline"
                  src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/check-box-outline-blank-1@2x.png"
                />
                <div className="frame-13">
                  <div className="frame-14">
                    <div className="frame-15">
                      <div className="text-wrapper-5">(주)엔미디어플랫폼</div>
                      <img
                        className="img"
                        alt="Favorite"
                        src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/favorite@2x.png"
                      />
                    </div>
                    <p className="p">등록 10/3(월) ~ 마감 10/25(금)</p>
                  </div>
                  <div className="frame-16">
                    <div className="frame-17">
                      <div className="frame-18">
                        <p className="text-wrapper-6">Nexon Company window application 개발 엔지니어 모집</p>
                        <img
                          className="grade"
                          alt="Grade"
                          src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/grade@2x.png"
                        />
                      </div>
                      <div className="frame-19">
                        <div className="frame-20">
                          <div className="text-wrapper-7">3년 경력</div>
                          <img
                            className="img"
                            alt="Arrow upward alt"
                            src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/arrow-upward-alt@2x.png"
                          />
                        </div>
                        <div className="frame-20">
                          <div className="text-wrapper-7">학력 무관</div>
                        </div>
                        <div className="frame-20">
                          <div className="text-wrapper-7">서울 강남구</div>
                        </div>
                        <div className="text-wrapper-7">정규직</div>
                      </div>
                    </div>
                    <div className="frame-22">
                      <div className="text-wrapper-8">마감된 공고</div>
                    </div>
                  </div>
                </div>
              </div>
              <img
                className="line"
                alt="Line"
                src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/line-2.png"
              />
              <div className="frame-12">
                <img
                  className="img"
                  alt="Check box outline"
                  src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/check-box-outline-blank-1@2x.png"
                />
                <div className="frame-13">
                  <div className="frame-14">
                    <div className="frame-15">
                      <div className="text-wrapper-5">(주)엔미디어플랫폼</div>
                      <img
                        className="img"
                        alt="Favorite"
                        src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/favorite@2x.png"
                      />
                    </div>
                    <p className="p">등록 10/3(월) ~ 마감 10/25(금)</p>
                  </div>
                  <div className="frame-16">
                    <div className="frame-17">
                      <div className="frame-18">
                        <p className="text-wrapper-6">Nexon Company window application 개발 엔지니어 모집</p>
                        <img
                          className="grade"
                          alt="Grade"
                          src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/grade@2x.png"
                        />
                      </div>
                      <div className="frame-19">
                        <div className="frame-20">
                          <div className="text-wrapper-7">3년 경력</div>
                          <img
                            className="img"
                            alt="Arrow upward alt"
                            src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/arrow-upward-alt@2x.png"
                          />
                        </div>
                        <div className="frame-20">
                          <div className="text-wrapper-7">학력 무관</div>
                        </div>
                        <div className="frame-20">
                          <div className="text-wrapper-7">서울 강남구</div>
                        </div>
                        <div className="text-wrapper-7">정규직</div>
                      </div>
                    </div>
                    <div className="frame-22">
                      <div className="text-wrapper-8">마감된 공고</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="frame-23">
        <div className="frame-24">
          <img
            className="account-circle"
            alt="Account circle"
            src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/account-circle@2x.png"
          />
          <div className="frame-25">
            <div className="frame-26">
              <div className="text-wrapper-9">백기석</div>
              <div className="text-wrapper-10">linsadfk0543</div>
            </div>
            <div className="frame-27">
              <div className="text-wrapper-11">내 정보 변경</div>
            </div>
          </div>
        </div>
        <img
          className="line-2"
          alt="Line"
          src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/line-18@2x.png"
        />
        <div className="frame-28">
          <div className="frame-29">
            <div className="text-wrapper-12">스크랩한 공고</div>
          </div>
          <div className="frame-29">
            <div className="text-wrapper-12">나의 활동</div>
            <div className="frame-29">
              <div className="frame-18">
                <div className="frame-30" />
                <div className="text-wrapper">내가 작성한 게시물</div>
              </div>
              <div className="frame-18">
                <div className="frame-30" />
                <div className="text-wrapper">내가 작성한 댓글</div>
              </div>
              <div className="frame-18">
                <div className="frame-30" />
                <div className="text-wrapper">좋아요 누른 게시물</div>
              </div>
            </div>
          </div>
        </div>
        <div className="frame-31">
          <img
            className="line-3"
            alt="Line"
            src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/line-18@2x.png"
          />
          <div className="text-wrapper-13">로그아웃</div>
        </div>
      </div>
      <div className="view">
        <div className="view-2">
          <div className="navbar">
            <div className="text-wrapper-14">AI 모의면접</div>
            <div className="text-wrapper-14">자기소개서 첨삭</div>
            <div className="text-wrapper-14">취업공고</div>
            <div className="text-wrapper-14">커뮤니티</div>
            <div className="text-wrapper-14">캘린더</div>
          </div>
          <div className="view-wrapper">
            <div className="view-3" />
          </div>
          <div className="div-2">
            <div className="frame-32">
              <p className="div-3">
                <span className="span">백기석</span>
                <span className="text-wrapper-15"> 님</span>
              </p>
              <img
                className="img"
                alt="Arrow drop down"
                src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/668681f71fc2293e52abea39/img/arrow-drop-down@2x.png"
              />
            </div>
            <img
              className="notifications"
              alt="Notifications"
              src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/6688fccfcda281749136af44/img/notifications@2x.png"
            />
          </div>
        </div>
      </div>
      <div className="view">
        <div className="view-2">
          <div className="navbar">
            <div className="text-wrapper-14">AI 모의면접</div>
            <div className="text-wrapper-14">자기소개서 첨삭</div>
            <div className="text-wrapper-14">취업공고</div>
            <div className="text-wrapper-14">커뮤니티</div>
            <div className="text-wrapper-14">캘린더</div>
          </div>
          <div className="view-wrapper">
            <div className="view-3" />
          </div>
          <div className="div-2">
            <div className="frame-33">
              <p className="div-4">
                <span className="span">백기석</span>
                <span className="text-wrapper-15"> 님</span>
              </p>
              <img
                className="img"
                alt="Arrow drop down"
                src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/668681f71fc2293e52abea39/img/arrow-drop-down@2x.png"
              />
            </div>
            <img
              className="notifications"
              alt="Notifications"
              src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/6688fccfcda281749136af44/img/notifications@2x.png"
            />
          </div>
        </div>
      </div>
      <div className="view-4">
        <div className="text-wrapper-16">알람</div>
        <div className="frame-34">
          <div className="text-wrapper-16">공고 마감 하루 전입니다.</div>
          <p className="text-wrapper-17">(주)엔미디어플랫폼 Nexon Company window application 개발 엔지니어 모집</p>
        </div>
        <img
          className="line-4"
          alt="Line"
          src="https://cdn.animaapp.com/projects/666f9293d0304f0ceff1aa2f/releases/66c2e3b54cbc5fc1778d008d/img/line-20@2x.png"
        />
        <div className="frame-34">
          <div className="text-wrapper-16">공고 마감 하루 전입니다.</div>
          <p className="text-wrapper-17">(주)엔미디어플랫폼 Nexon Company window application 개발 엔지니어 모집</p>
        </div>
      </div>
    </div>
  );
};
